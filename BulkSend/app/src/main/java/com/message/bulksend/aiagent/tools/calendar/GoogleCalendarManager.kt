...
class GoogleCalendarManager(private val context: Context) {
    ...
    private val tag = "GoogleCalendarManager"
    ...
    val workerBaseUrl: String
        get() = GoogleCalendarAgentTool.WORKER_URL.trimEnd('/')
        ...
        return try {
            ...
            if (!isWorkerConfigured()) {
                return Result.failure(
                    IllegalStateException("Google Calendar worker URL is not configured")
                )
            }
    ...
    suspend fun getConfig(): CalendarConfig? {
        return try {
            ...
            CalendarConfig(
                uid = userId,
                calendarApiConnected = snapshot.getBoolean("calendarApiConnected") ?: false,
                calendarApiLastSetup = snapshot.getString("calendarApiLastSetup") ?: "",
                googleCalendarConfig = snapshot.getString("googleCalendarConfig") ?: ""
            )
        } catch (e: Exception) {
        ...
        try {
            ...
            if (userId.isNotBlank()) {
                firestore.collection("users_config").document(userId)
                    .update(
                        mapOf(
                            "calendarApiConnected" to false,
                            "googleCalendarConfig" to com.google.firebase.firestore.FieldValue.delete()
                        )
                    ).await()
            }
        } catch (e: Exception) {
    ...
    suspend fun isSetupDone(): Boolean = getConfig()?.calendarApiConnected == true
}

data class CalendarConfig(
    val uid: String,
    val calendarApiConnected: Boolean,
    val calendarApiLastSetup: String,
    val googleCalendarConfig: String
)
